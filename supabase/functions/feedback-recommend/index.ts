...
- 아트패스 DB에 등록된 대학 목록 기준으로만 추천합니다.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const userPrompt = `${n > 1 ? `${n}장의 실기 작품이 제공되었습니다. 전체 작품을 종합적으로 분석하여` : '이 실기 작품을 보고,'} 아트패스에 현재 등록된 대학 목록 중 이 학생에게 가장 잘 맞는 대학을 추천해주세요.

[아트패스 등록 대학 목록 — 실시간 데이터]
${uniSummary}
